...
Java provides a package for \nom{Remote Method Invocation}{RMI}\footnote{\url{http://docs.oracle.com/javase/7/docs/api/java/rmi/package-summary.html\#package_description}}, which allows object in one Java VM to invoke methods on objects in another Java VM.
...
As a second alternative, we could communicate with messaging by using message-oriented middleware.
As explained earlier in \autoref{fundamentals:service} it support communication between different components using adapters and channels.
It also provides asynchronous communication by using message queues for temporary storage and often provides even additional persistent storage and backups for high availability~\autocite{mom}. \textcolor{red}{security/encryption?}
