...
In step one, the bootware call an infrastructure plugin to create a new infrastructure instance, for example a VM.
...
Here,  provision workflow middleware plugin can call this application, in our case a provisioning engine, to provision a workflow middleware.
...
Here, a Chef server is installed on a Azure VM over a \nom{remote desktop connection}{RDC} connection.
...
Infrastructure plugins are responsible for provisioning any computing resources that the user wants to use during the bootware process.
