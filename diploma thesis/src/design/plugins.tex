...
In most cases this will not be necessary, because the instance will be destroyed in the undeploy phase and with it all the payload data (assuming it was not installed in persistent storage).
...
\begingroup
	...
	\end{tabu}
	\caption{Interface to be implemented by call provisioning engine plugins.}
...
Since these plugins offer identical functionality, it makes sense to use the same plugins for both the bootware and the provisioning manager.
...
The call provisioning manager plugins are described in more detail in~\autocite{nedim} because they are only a small part of the bootware and are mainly used in the provisioning manager.
