...
We add two additional operations that we also need, the \textit{getActivePayloads} operation and the \textit{shutdown} operation.
Both the local and the remote bootware will have to implement all of these operations, except the \textit{getActivePayloads} operation, which is only needed in the remote bootware.
