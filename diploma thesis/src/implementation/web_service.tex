...
In line 7-23, it contains a credentials list, which can contain one or more credentials set.
Each credentials set is made up of one or more credentials, which are key value pairs, where the key describes the credential type and the value the actual credential.
In the example code in line 9, we send one credentials set for AWS, which consists of two credentials, a secret key in line 12-15 and an accessKey in line 16-19.
...
\begin{listing}[!htbp]
	\inputminted[
		...
		framesep=0.3cm,
		fontsize=\small
	]{xml}{implementation/assets/setCredentials-request.xml}
	\caption{Sample setCredentials request in a soap message.}
	\label{lst:webservice:setcredentialsrequest}
\end{listing}

If the setCredentials operation was successful, the response in \autoref{lst:webservice:setcredentialsresponse} will be returned.
...
\begin{listing}[!htbp]
	\inputminted[
		...
		framesep=0.3cm,
		fontsize=\small
	]{xml}{implementation/assets/setCredentials-response.xml}
	\caption{Sample setCredentials response in a soap message.}
	\label{lst:webservice:setcredentialsresponse}
\end{listing}

Like the deploy and undeploy operations, the setCredentials operation also returns an error message if the operation failed.
As can be seen in \autoref{lst:webservice:setcredentialserror}, it also contains a soap fault string in line 5 and the original SetCredentialsException thrown by the setCredentials operation in line 7-10.

\vspace*{\baselineskip}
\begin{listing}[!htbp]
	\inputminted[
		label=setCredentials-error.xml,
		frame=topline,
		linenos,
		frame=lines,
		tabsize=2,
		framesep=0.3cm,
		fontsize=\small
	]{xml}{implementation/assets/setCredentials-error.xml}
	\caption{Sample setCredentials error in a soap message.}
	\label{lst:webservice:setcredentialserror}
\end{listing}
