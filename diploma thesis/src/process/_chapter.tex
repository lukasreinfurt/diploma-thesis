...
Once the workflow middleware is up and running, the provisioning engine returns the information about the workflow middleware, such as endpoint referencens, to the remote bootware, which in turn returns it to the local bootware, which returns it to the bootware adapter.
