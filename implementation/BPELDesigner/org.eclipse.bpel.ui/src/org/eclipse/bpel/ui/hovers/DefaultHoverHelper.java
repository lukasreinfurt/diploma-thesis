...
public class DefaultHoverHelper implements IHoverHelper {
	...
	public IFigure getHoverFigure(EObject modelObject) {
		...
		//@hahnml: Show the xPath of the element if it has one
		if (modelObject instanceof BPELExtensibleElement) {
			BPELExtensibleElement element = (BPELExtensibleElement) modelObject;
			
			if (element.getXPath() != null && !element.getXPath().isEmpty()) {
				panel.add(new Label("XPath :"));
				panel.add(new Label(element.getXPath()));
			}
		}
