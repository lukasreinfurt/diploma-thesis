...
/**
 * This is the superclass of BPEL activities that are leaves. That is,
 * activities which don't contain other activities. Leaf activities currently
 * are laid out in a horizontal manner with icon and label, with a simple border
 * surrounding.
 */
public class LeafEditPart extends ActivityEditPart {
	...
	// @hahnml: A new figure which holds all other figures and the colored background.
	protected Panel colorFigure;
	...
	public static class LeafDecorationLayout extends BPELDecorationLayout {
		...
		protected Point calculateLocation(int locationHint, IFigure container,
			...
			Point result = super.calculateLocation(locationHint, container,
					childDimension);
			...
			// if it is somewhere on the right we need to remove the drawer
			// space
	...
	protected void createEditPolicies() {
		...
		installEditPolicy(EditPolicy.SELECTION_FEEDBACK_ROLE,
				new BPELSelectionEditPolicy(false, true) {
					@Override
	...
	protected IFigure createFigure() {
		...
		this.border = new LeafBorder(editPartMarkerDecorator
				.getDecorationLayer(), false);
	...
	protected GradientFigure getContentFigure() {
		return (GradientFigure) this.contentFigure;
	}
	...
	public void refreshVisuals() {
		...
		// @hahnml
		this.colorFigure
				.setBackgroundColor(AgoraHelper.colorFigure(getModel()));
	...
	protected void unregisterVisuals() {
		...
		if (editPartMarkerDecorator != null) {
			editPartMarkerDecorator
					.removeMarkerMotionListener(markerMotionListener);
		}
		this.image = null;
	...
	// return new CenteredConnectionAnchor(getMainActivityFigure(),
	// CenteredConnectionAnchor.BOTTOM, 0);
	// }
	//	
	//	
	//
	...
	// // However, they both anchor at the same part of the edit part.
	// if (anc == null)
	// anc = new CenteredConnectionAnchor(getMainActivityFigure(),
	// CenteredConnectionAnchor.TOP, 0);
	// return anc;
	// }
	@Override
