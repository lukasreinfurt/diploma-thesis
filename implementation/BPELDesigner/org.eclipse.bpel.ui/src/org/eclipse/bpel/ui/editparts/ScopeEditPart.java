...
public class ScopeEditPart extends CollapsableEditPart {
	...
	public Label getLabelFigure() {
		return getScopeBorder().getLabel();
	}
	 ...
	 * with an offset of 0.
	 * 
	 * @hahnml: In case that the figure which holds the image and the label is always displayed, the offset has to be greater than before.
	 */
	@Override
	public ConnectionAnchor getConnectionAnchor(int location) {
		switch(location){
		case CenteredConnectionAnchor.TOP_INNER:
			return new CenteredConnectionAnchor(contentFigure, location, 30);
