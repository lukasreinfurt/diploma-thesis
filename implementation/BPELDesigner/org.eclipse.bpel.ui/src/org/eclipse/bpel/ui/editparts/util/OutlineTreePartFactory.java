...
public class OutlineTreePartFactory implements EditPartFactory {
	...
	public static List getModelChildren(Object model) {
		...
		if (model instanceof Process) {
			...
			Object[] rootItems = {process.getPartnerLinks(), process.getVariables(), process.getReferenceVariables(), process.getContainerReferenceVariables(), process.getDataSourceReferenceVariables(), process.getCorrelationSets(), process.getMessageExchanges()};
			...
			for (Iterator iter = children.iterator(); iter.hasNext();) {
				...
				if (!(element instanceof PartnerLink || element instanceof Variable || element instanceof ReferenceVariable || element instanceof ContainerReferenceVariable || element instanceof DataSourceReferenceVariable || element instanceof CorrelationSet || element instanceof MessageExchange)) {
					result.add(element);
				}
			}
			return result;
