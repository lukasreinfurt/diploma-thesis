...
public class PageMessageExchange extends FragmentWizardPage {

	private TableViewer viewer = null;
	
	public PageMessageExchange(IWorkbench workbench,
			IStructuredSelection selection, WizardModel model) {
		super(workbench, selection, model, "Page4",
				"Handle MessageExchange Conflicts",
				"Solve the message exchange conflicts");
	}
	...
	public void createControl(Composite parent) {
		...
		group.setText("Existing process message exchanges");
		...
		List list = new List(group, SWT.SINGLE | SWT.BORDER | SWT.V_SCROLL
				| SWT.H_SCROLL);
		if (this.model.process.getMessageExchanges() != null) {
			list.setItems(FragmentUtils.getNameArrayFromEbjectList(
					this.model.process.getMessageExchanges().getChildren(),
					BPELPackage.Literals.MESSAGE_EXCHANGE__NAME));
		}
		...
		ConflictTable confTable = new ConflictTable(
				ConflictTable.TableType.MESSAGE_EXCHANGE, tableViewer);
		...
		tableViewer.setInput(FragmentUtils.getUtils(null).getConflictsOfType(
				BPELPackage.Literals.MESSAGE_EXCHANGE));
