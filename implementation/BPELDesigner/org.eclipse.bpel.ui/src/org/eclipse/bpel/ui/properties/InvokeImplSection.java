...
//	}
	...
	private void findAndSetOperation (String text) {
		...
		if (text.length() > 0) {
			...
			//@sonntamo: find the edit part for this invoke activity and set a new icon if needed.
			Map reg = getBPELEditor().getGraphicalViewer().getEditPartRegistry();
			Object editPart= reg.get(model);
			if (editPart instanceof InvokeEditPart) {
				InvokeEditPart invokeEP = (InvokeEditPart)editPart;
				invokeEP.setNewIcon(op);
			}
...
//		
		...
		for (int i=0,j=path.getSegmentCount(); i < j; i++) {
			...
			if (model instanceof PartnerLink) {
				...
				//@sonntamo: get the edit part of the invoke activity and set a new icon
				Map reg = getBPELEditor().getGraphicalViewer().getEditPartRegistry();
				Object editPart= reg.get(input);
				if (editPart instanceof InvokeEditPart) {
					InvokeEditPart invokeEP = (InvokeEditPart)editPart;
					invokeEP.setNewIcon(op);
				}
	...
	private List<Command> basicCommandList (EObject input, PartnerLink pl, Operation op) {
		...
		if (op != IGNORE_OPERATION) {
			...
			//@sonntamo: get the edit part of this invoke activity and set a new icon
			Map reg = getBPELEditor().getGraphicalViewer().getEditPartRegistry();
			Object editPart= reg.get(input);
			if (editPart instanceof InvokeEditPart) {
				InvokeEditPart invokeEP = (InvokeEditPart)editPart;
