...
public class PageCorrelationSet extends FragmentWizardPage {

	private TableViewer viewer = null;
	
	public PageCorrelationSet(IWorkbench workbench, IStructuredSelection selection,
			WizardModel model) {
		super(workbench, selection, model, "Page3", "Handle CorrelationSet Conflicts",
				"Solve the correlationSet conflicts");
	}
	...
	public void createControl(Composite parent) {
	    ...
	    group.setText("Existing process correlation sets");
		...
		if (this.model.process.getCorrelationSets() != null) {
			list.setItems(FragmentUtils.getNameArrayFromEbjectList(this.model.process.getCorrelationSets().getChildren(), BPELPackage.Literals.CORRELATION_SET__NAME));
		}

		// Create and setup the TableViewer
		final TableViewer tableViewer = ConflictTableLabelProvider.createTableViewer(table);
		ConflictTable confTable = new ConflictTable(ConflictTable.TableType.CORRELATION_SET, tableViewer);
		
		//@hahnml: Register the current viewer in FragmentUtils and this page as listener
		...
		tableViewer.setInput(FragmentUtils.getUtils(null).getConflictsOfType(BPELPackage.Literals.CORRELATION_SET));
		...
		buttonComp.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, true, false, 3, 1));
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (selection != null && selection instanceof IStructuredSelection) {
					...
					if (sel.size() == 1) {
						...
						ConflictSolveDialog dialog = new ConflictSolveDialog(tableViewer.getTable().getShell(), conflict);
						dialog.open();
					}
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (selection != null && selection instanceof IStructuredSelection) {
					...
					if (sel.size() == 1) {
						...
						FragmentUtils.getUtils(null).removeConflictedElement(conflict);
					}
	...
	public boolean canFlipToNextPage() {
		...
		//@hahnml: Only allowed if no conflicts exist or all conflicts are solved
		if (!FragmentUtils.getUtils(null).hasConflictsOfType(BPELPackage.Literals.CORRELATION_SET)) {
			canFlip = true;
		}
