 ...
 * 
 */
public class ProcessTrayEditPart extends MainTrayEditPart implements
		IHoverHelperSupport {
	...
	protected EditPartMarkerDecorator createEditPartMarkerDecorator() {
		...
		return new TrayMarkerDecorator((EObject) getModel(),
				new ToolbarLayout()) {
			@Override
			protected IMarker[] getMarkers() {

				IMarkerHolder holder = BPELUtil.adapt(modelObject,
						IMarkerHolder.class);
	...
	protected void createEditPolicies() {
		...
		// The DIRECT_EDIT_ROLE policy determines how in-place editing takes
		// place.
		installEditPolicy(EditPolicy.DIRECT_EDIT_ROLE,
				new BPELDirectEditPolicy());
	}
	...
	protected PartnerLinks getPartnerLinks() {
		if (lastSelection instanceof Scope) {
			...
			if (scope.getPartnerLinks() == null) {
				scope.setPartnerLinks(BPELFactory.eINSTANCE
						.createPartnerLinks());
			}
	...
	/**
	 * We show scoped variables if a Scope is the current selection, otherwise
	 * we show the process variables.
	 */
	...
	protected ReferenceVariables getReferenceVariables() {
		...
		if (process.getReferenceVariables() == null) {
			process.setReferenceVariables(BPELFactory.eINSTANCE
	...
	protected ContainerReferenceVariables getContainerReferenceVariables() {
		...
		if (process.getContainerReferenceVariables() == null) {
			process.setContainerReferenceVariables(BPELFactory.eINSTANCE
					.createContainerReferenceVariables());
		}
	...
	protected DataSourceReferenceVariables getDataSourceReferenceVariables() {
		...
		if (process.getDataSourceReferenceVariables() == null) {
			process.setDataSourceReferenceVariables(BPELFactory.eINSTANCE
					.createDataSourceReferenceVariables());
		}
	...
	protected CorrelationSets getCorrelationSets() {
		...
		if (process.getCorrelationSets() == null) {
			process.setCorrelationSets(BPELFactory.eINSTANCE
					.createCorrelationSets());
		}
	...
	protected MessageExchanges getMessageExchanges() {
		if (lastSelection instanceof Scope) {
			...
			if (scope.getMessageExchanges() == null) {
				scope.setMessageExchanges(BPELFactory.eINSTANCE
		...
		if (process.getMessageExchanges() == null) {
			process.setMessageExchanges(BPELFactory.eINSTANCE
					.createMessageExchanges());
		}
	...
	public void activate() {
		...
		editor.getGraphicalViewer().addSelectionChangedListener(
	...
	public void deactivate() {
		super.deactivate();
		// There is a chance that by the time we deactivate, we can't find the
		// editor anymore.
		// This is okay because there is a hack in BPELEditor.modelReloaded that
		// manually
		// removes this selection change listener from the graphical viewer.
		try {
			BPELEditor editor = ModelHelper.getBPELEditor(getProcess());
			editor.getGraphicalViewer().removeSelectionChangedListener(
					getSelectionChangedListener());
		} catch (Exception e) {
		}
	}
	...
	/**
	 * Selection listeners that tracks the canvas selection and causes the
	 * variables to refresh accordingly.
	 * 
		...
		if (selectionListener == null) {
			selectionListener = new ISelectionChangedListener() {
				public void selectionChanged(SelectionChangedEvent event) {
					Object currentSelection = getModelObjectFromSelection(event
							.getSelection());
				...
				protected boolean shouldRefresh(Object currentSelection) {
					if (currentSelection == null)
						return false;
					if (currentSelection instanceof Variables
							|| currentSelection instanceof Variable)
						return false;
					if (currentSelection instanceof ReferenceVariables
							|| currentSelection instanceof ReferenceVariable)
						return false;
					if (currentSelection instanceof PartnerLinks
							|| currentSelection instanceof PartnerLink)
						return false;
					if (currentSelection instanceof CorrelationSets
							|| currentSelection instanceof CorrelationSet)
						return false;
					if (currentSelection instanceof MessageExchanges
							|| currentSelection instanceof MessageExchange)
						return false;
