...
public class BPELEditModelClient extends EditModelClient {
	...
	// @hahnml
	private ResourceInfo debugResourceInfo;
	...
	/**
	 * Load the debug resource (<process-name>.debug file)
	 * @throws RuntimeException
	 */
	public void loadDebugResource() throws RuntimeException {

		if (bpelEditModel==null || getPrimaryResourceInfo()==null)
			throw new RuntimeException("Primary resource must be loaded before Debug");
		
		// @hahnml
		IFile debugFile = bpelEditModel.getDebugFile();
		Map<String, Object> options = new HashMap<String, Object>();
		options.put(XMLResource.OPTION_ENCODING, "UTF-8");
		
		if (debugFile.exists()) {
			debugResourceInfo = bpelEditModel.getResourceInfo(debugFile);
			debugResourceInfo.setLoadOptions(options);
		} else {
			Resource debugResource = bpelEditModel.getResourceSet()
					.createResource(
							URI.createPlatformResourceURI(debugFile
									.getFullPath().toString()));
			debugResourceInfo = bpelEditModel.getResourceInfo(debugFile);
			debugResourceInfo.setLoadOptions(options);
			// create a debug object and set global information.
			DebugFactory dFactory = DebugFactory.eINSTANCE;
			Debug debug = dFactory.createDebug();
			
			EList bpelContents = getPrimaryResourceInfo().getResource()
					.getContents();
			if (!bpelContents.isEmpty()
					&& bpelContents.get(0) instanceof Process) {
				Process process = (Process) bpelContents.get(0);
				
				debug.setProcessName(process.getName());
				debug.setProcessPath(BPELUtil.getBPELFile(process).getLocation().toOSString());
			}
			//Create the default contents of the debug resource
			debug.setGlobalBreakpoints(dFactory.createBreakpoints());
			debug.setLocalBreakpoints(dFactory.createBreakpoints());
			
			debugResource.getContents().add(debug);
		}
	}
	...
	public void dispose() {
		...
		// @hahnml
		if (debugResourceInfo != null) {
			bpelEditModel.releaseReference(debugResourceInfo);
		}
		super.dispose();
	}
	...
	public ResourceInfo getArtifactsResourceInfo() {
		return artifactsResourceInfo;
	}

	public ResourceInfo getExtensionsResourceInfo() {
		return extensionsResourceInfo;
	}

	// @hahnml
	public ResourceInfo getDebugResourceInfo() {
		return debugResourceInfo;
	}
	...
	public IFile getDebugFile() { return bpelEditModel.getDebugFile(); } 
}
