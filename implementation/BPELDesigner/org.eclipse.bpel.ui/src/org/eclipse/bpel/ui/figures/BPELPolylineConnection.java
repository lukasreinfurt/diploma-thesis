...
public class BPELPolylineConnection extends PolylineConnection {
	...
	private static final int LEFT_RIGHT = 0, RIGHT_LEFT = 1, TOP_BOTTOM = 2,
			BOTTOM_TOP = 3;

	Color color1, color2, color3;
	Image topLeft, topRight, bottomLeft, bottomRight, arrowDown, arrowRight;

	// @hahnml: New color to monitor link events
	Color linkStateColor;

	public BPELPolylineConnection(Image topLeft, Image topRight,
			Image bottomLeft, Image bottomRight, Image arrowDown,
			Image arrowRight) {
		...
		// Leave room for the 3-pixel wide line, as well as our custom
		// arrowhead.
		setLineWidth(12);
	}
	...
	protected void outlineShape(Graphics g) {
		...
		for (int i = 0; i < size - 1; i++) {
			...
			// Draw the line.
			// If we are on the last one, the line should fall short by 4
			// pixels.
			if (i == size - 2) {
				if (direction == RIGHT_LEFT || direction == LEFT_RIGHT)
					p2.x -= 4;
			...
			// hahnml: Check if a MonitorManager exists else use the default
			// color for the link
			if (MonitoringProvider.getInstance().getActiveProcessManager() != null
					&& MonitoringProvider.getInstance()
							.getActiveProcessManager().getLastStartedInstance() != null) {
				g.setForegroundColor(linkStateColor);
			} else {
			...
			if (direction == BOTTOM_TOP || direction == TOP_BOTTOM) {
				...
				// Horizontal
				g.setForegroundColor(color1);
				g.drawLine(p1.x, p1.y - 2, p2.x, p2.y - 2);
				g.setForegroundColor(color3);
				g.drawLine(p1.x, p1.y + 2, p2.x, p2.y + 2);
			}
			...
			if (i != 0) {
				...
				// int x = p1.x - 1;
				// int y = p1.y - 1;
				g.drawImage(image, p1.x - 2, p1.y - 2);
			}
	 ...
	 * @author hahnml
	 * 
	 * @param linkStateColor
	 *            The color to represent the state of the link
	 */
	public void setLinkStateColor(Color linkStateColor) {
		this.linkStateColor = linkStateColor;
	}
}
