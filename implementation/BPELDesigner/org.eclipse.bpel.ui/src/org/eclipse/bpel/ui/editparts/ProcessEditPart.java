...
public class ProcessEditPart extends BPELEditPart implements ILayoutAware {
	...
	// @hahnml: Holds the instance state figure.
	private IFigure instanceHolder;
	...
	private class ProcessOrderedHorizontalLayoutEditPolicy extends
			ProcessEditPart.ProcessOrderedLayoutEditPolicy {
		@Override
			...
			for (int i = 0; i < children.size(); i++) {
				if (i != children.size() - 1) {
					...
					if (sourcePart instanceof StartNodeEditPart) {
						boolean horizontal = ModelHelper
								.isHorizontalLayout(getHost().getModel());
						connection
	...
	public class ProcessOrderedLayoutEditPolicy extends
			BPELOrderedLayoutEditPolicy {
		...
		protected List getConnectionChildren(BPELEditPart editPart) {
			...
			while (it.hasNext()) {
				...
				if (next instanceof FaultHandlerEditPart) {
					continue;
				} else if (next instanceof InstanceStateNodeEditPart) {
					// @hahnml: InstanceStateNodeEditPart should not be
					// connected to other figures so we have to ignore it here.
					continue;
				}
	...
	protected void createEditPolicies() {
		...
		installEditPolicy(EditPolicy.CONTAINER_ROLE,
				new BPELContainerEditPolicy());

		// The process must lay out its child activity
		if (ModelHelper.isHorizontalLayout(getModel()))
			installEditPolicy(EditPolicy.LAYOUT_ROLE,
					new ProcessOrderedHorizontalLayoutEditPolicy());
		else
			installEditPolicy(EditPolicy.LAYOUT_ROLE,
					new ProcessOrderedLayoutEditPolicy());
	}
	...
	protected IFigure createFigure() {
		...
		// @hahnml: Here we create the instanceHolder figure in which we add the
		// instanceState figure later on. The instanceHolder figure acts like a
		// panel or composite.
		this.instanceHolder = new Figure();
		FlowLayout lay = new FlowLayout();
		lay.setMajorAlignment(FlowLayout.ALIGN_LEFTTOP);
		this.instanceHolder.setLayoutManager(lay);
	...
	protected IFigure getContentPane(EditPart childEditPart) {
		if (childEditPart instanceof StartNodeEditPart) {
			...
			return handlerHolder;
		} else if (childEditPart instanceof InstanceStateNodeEditPart) {
	...
	protected List getModelChildren() {
		...
		/*
		 * NOTE: The layoutEditPolicy relies on this order to identify the area
		 * the user is mousing over. Do not change the order in which these
		 * children are added unless you change the layoutEditPolicy
		 * too!!!!!!!!!!!!!!!!!!!!
		 */

		// TODO: this is way too magic. can we get rid of this?

		// @hahnml: We have to add the instanceState from the model.
		list.add(domain.getInstanceNode());

		list.add(domain.getStartNode());
		IContainer container = new ActivityContainer(BPELPackage.eINSTANCE
				.getProcess_Activity());
	...
	public FaultHandler getFaultHandler() {
		IFaultHandlerHolder holder = BPELUtil.adapt(getModel(),
	...
	public EventHandler getEventHandler() {
		IEventHandlerHolder holder = BPELUtil.adapt(getModel(),
				IEventHandlerHolder.class);
		if (holder != null) {
	...
	protected void addChildVisual(EditPart childEditPart, int index) {
		...
		if (content != null) {
			if (content == activityHolder) {
				// @hahnml: Index decreased now by two (-2)
				// The index includes the start node and the instance node,
				// which aren't in this
				// content pane. Subtract two.
				content.add(child, index - 2);
			} else if (content == handlerHolder) {
				content.add(child, getIndexForChild(content, childEditPart));
			} else if (content == instanceHolder) {
				// @hahnml: Add the instanceState figure to the content pane.
				content.add(child, 0);
			}
		}
	}
	...
	/**
	 * Gets the index for the given <code>child</code> which should be inserted
	 * into the <code>container</code> afterwards. This method does a kind of
	 * really simple sorting.
	 * 
	 * @param container
	 *            The container where the child will be inserted.
	 * @param child
	...
	public void performRequest(Request request) {
		if (request.getType() == RequestConstants.REQ_DIRECT_EDIT) {
			// let's not activate the rename or direct edit functionality
			// because it's annoying when
			...
			protected void handleFinished() {
				if (getViewer().getSelection().isEmpty()) {
					...
					getViewer().setSelection(
							new StructuredSelection(ProcessEditPart.this));
				}
			}
		};
	}
	...
	public void switchLayout(boolean horizontal) {
		AlignedFlowLayout handlerLayout = (AlignedFlowLayout) handlerHolder
				.getLayoutManager();
		handlerLayout.setHorizontal(!horizontal);

		// Adjust the layout of the activityAndHandlerHolder
		((AlignedFlowLayout) activityAndHandlerHolder.getLayoutManager())
				.setHorizontal(horizontal);

		// Adjust the layout of the activityHolder
		((AlignedFlowLayout) activityHolder.getLayoutManager())
				.setHorizontal(horizontal);

		// Remove the LayoutPolicy which is responsible for creating the
		// implicit links
		...
		if (horizontal) {
			...
			this.instanceHolder.setBorder(new MarginBorder(15, 0, 0, 0));
