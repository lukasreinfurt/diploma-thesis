...
public class PagePartnerLinkConflicts extends FragmentWizardPage {

	private TableViewer viewer = null;
	
	public PagePartnerLinkConflicts(IWorkbench workbench,
			IStructuredSelection selection, WizardModel model) {
		super(workbench, selection, model, "Page2",
				"Handle PartnerLink Conflicts",
				"Solve the partnerLink conflicts");
	}
	...
	public void createControl(Composite parent) {
		...
		group.setText("Existing process partnerLinks");
		...
		List list = new List(group, SWT.SINGLE | SWT.BORDER | SWT.V_SCROLL
				| SWT.H_SCROLL);
		if (this.model.process.getPartnerLinks() != null) {
			list.setItems(FragmentUtils.getNameArrayFromEbjectList(
					this.model.process.getPartnerLinks().getChildren(),
					BPELPackage.Literals.PARTNER_LINK__NAME));
		}
		...
		ConflictTable confTable = new ConflictTable(
				ConflictTable.TableType.PARTNERLINK, tableViewer);
		...
		tableViewer.setInput(FragmentUtils.getUtils(null).getConflictsOfType(
				BPELPackage.Literals.PARTNER_LINK));
