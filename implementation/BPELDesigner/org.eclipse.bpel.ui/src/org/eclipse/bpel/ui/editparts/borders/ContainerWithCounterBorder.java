...
/**
 * This is the border for while, forEach and repeatUntil.
 * 
 * When expanded it displays a label, a text and a counter at the top as well as showing the
 * contents.
 * 
 * @author hahnml
 */
public class ContainerWithCounterBorder extends ContainerBorder {
	...
	protected Label collapsedCounterLabel = null;
	...
	private Rectangle rectLabel, rectImageLabel, rectCounterLabel;

	// The bounds of the round rectangle surrounding the label, image and counter 
	// when expanded
	...
	public ContainerWithCounterBorder(IFigure parentFigure, Image image, String labelText, String counterText) {
		...
		this.collapsedCounterLabel = new Label(counterText);
		this.collapsedCounterLabel.setFont(JFaceResources.getFontRegistry().get(JFaceResources.DEFAULT_FONT));
	}
	
	public void setCounter(String counter) {
		collapsedCounterLabel.setText(counter);
	}
	...
	protected void doPaint(IFigure figure, Graphics graphics, Insets insets) {
		...
		collapsedCounterLabel.setBounds(rectCounterLabel);
		collapsedCounterLabel.paint(graphics);
	...
	protected void calculate(IFigure figure) {
		...
		// preferred size of the counter label
		Dimension counterLabelSize = collapsedCounterLabel.getPreferredSize().getCopy();
		
		// calculate the label border for the round rectangle
		// surrounding the label and image
		int w = labelSize.width + spacing + imageLabelSize.width + spacing + counterLabelSize.width;
		int h = Math.max(Math.max(labelSize.height, imageLabelSize.height), counterLabelSize.height);
		...
		// rectangle for counter label	
		x = rectLabelBorder.x + leftMargin + imageLabelSize.width + spacing + labelSize.width + spacing;
		y = rectLabelBorder.y + topMargin;
		w = counterLabelSize.width;
		h = counterLabelSize.height;
		rectCounterLabel = new Rectangle(x, y, w, h);
