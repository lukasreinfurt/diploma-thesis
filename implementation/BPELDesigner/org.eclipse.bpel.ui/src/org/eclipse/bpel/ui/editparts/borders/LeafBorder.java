...
public class LeafBorder extends GradientBorder {
	...
	// @hahnml: We need a LeafBorder without margins to color the figures, but
	// some other activity figures need the margins so we have to handle this
	// with a boolean.
	private boolean withMargins = true;
	...
	public LeafBorder(IFigure parentFigure, boolean withMargins) {
	...
	public void paint(IFigure figure, Graphics graphics, Insets insets) {
		...
		graphics.setForegroundColor(registry
				.get(IBPELUIConstants.COLOR_ACTIVITY_BORDER));
		...
		if (topMarker != null || bottomMarker != null) {
			...
			if (bottomMarker == null) {
				clippingRect = new Rectangle(bounds.x, bounds.y, DRAWER_WIDTH,
						DRAWER_HALF_HEIGHT + 1);
			} else if (topMarker == null) {
				clippingRect = new Rectangle(bounds.x, bounds.y
						+ DRAWER_HALF_HEIGHT, DRAWER_WIDTH,
						DRAWER_HALF_HEIGHT + 1);
			} else {
				clippingRect = new Rectangle(bounds.x, bounds.y, DRAWER_WIDTH,
						DRAWER_HEIGHT + 1);
			...
			// -1 due to GEF
			graphics.drawRoundRectangle(new Rectangle(bounds.x + DRAWER_INSET,
					bounds.y, DRAWER_WIDTH * 2, DRAWER_HEIGHT - 1),
					IBPELUIConstants.ARC_WIDTH, IBPELUIConstants.ARC_WIDTH);
			graphics.setClip(oldClip);
			if (bottomMarker == null || topMarker == null) {
				graphics.drawLine(bounds.x + DRAWER_INSET, bounds.y
						+ DRAWER_HALF_HEIGHT, bounds.x + DRAWER_WIDTH, bounds.y
						+ DRAWER_HALF_HEIGHT);
			}
		}
		...
		if (needSquareCorners) {
			Rectangle clippingRect = new Rectangle(rectBounds.x
					+ rectBounds.width / 2, rectBounds.y,
					rectBounds.width / 2 + 2, rectBounds.height + 1);
			graphics.setClip(clippingRect);
			graphics.drawRoundRectangle(rectBounds, arcSize, arcSize);
			clippingRect = new Rectangle(rectBounds.x, rectBounds.y,
					rectBounds.width / 2 + 1, rectBounds.height + 1);
			graphics.setClip(clippingRect);
		...
		computeHandlerIconPositions(ModelHelper
				.isHorizontalLayout(getEditPart().getModel()));
	...
	public Insets getInsets(IFigure figure) {
		//@hahnml: Here we have to check if we need margins or not.
		if (withMargins) {
			return new Insets(borderWidth + topMargin, borderWidth + leftMargin
					+ DRAWER_WIDTH, borderWidth + bottomMargin, borderWidth
					+ rightMargin + DRAWER_WIDTH);
		} else {
			return new Insets(borderWidth, borderWidth + DRAWER_WIDTH,
					borderWidth, DRAWER_WIDTH);
		}
	}
	...
	public boolean isPointInTopDrawer(int x, int y) {
		...
		Rectangle imageBounds = new Rectangle(imageX, imageY, imageSize.width,
				imageSize.height);
	...
	private void computeHandlerIconPositions(boolean horizontal) {
		if (horizontal) {
			...
			// rectangle
			if (showFault) {
				int x = bounds.x + bounds.width - faultImageWidth
						- DRAWER_WIDTH + 1;
				if (showCompensation)
					x -= compensationImageWidth;
				if (showEvent)
					x -= eventImageWidth;
				this.rectFault = new Rectangle(x, getGradientRect().y
						+ getGradientRect().height - faultImageHeight
						+ yHandlerIconOffset, faultImageWidth, faultImageHeight);
			}
			// Draw the compensation image in the upper right hand corner of the
			// round rectangle
			if (showCompensation) {
				int x = bounds.x + bounds.width - compensationImageWidth
						- DRAWER_WIDTH + 1;
				if (showEvent)
					x -= eventImageWidth;
				this.rectCompensation = new Rectangle(x, getGradientRect().y
						+ getGradientRect().height - compensationImageHeight
						+ yHandlerIconOffset, compensationImageWidth,
						compensationImageHeight);
			}
			if (showEvent) {
				int x = bounds.x + bounds.width - eventImageWidth
						- DRAWER_WIDTH + 1;
				this.rectEvent = new Rectangle(x, getGradientRect().y
						+ getGradientRect().height - eventImageHeight
						+ yHandlerIconOffset, eventImageWidth, eventImageHeight);
			}
		} else {
			// Draw the fault image in the upper right hand corner of the round
			// rectangle
			if (showFault) {
				this.rectFault = new Rectangle(bounds.x + bounds.width
						- faultImageWidth - DRAWER_WIDTH + 1, bounds.y,
			...
			if (showCompensation) {
				...
				this.rectCompensation = new Rectangle(bounds.x + bounds.width
						- compensationImageWidth - DRAWER_WIDTH + 1,
						compensationImageOffset, compensationImageWidth,
						compensationImageHeight);
			}
			if (showEvent) {
				...
				this.rectEvent = new Rectangle(bounds.x + bounds.width
						- eventImageWidth - DRAWER_WIDTH + 1, eventImageOffset,
