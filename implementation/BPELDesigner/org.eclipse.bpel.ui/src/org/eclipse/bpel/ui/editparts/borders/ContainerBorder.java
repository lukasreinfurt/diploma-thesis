...
public class ContainerBorder extends CollapsableBorder {
	...
	protected void doPaint(IFigure figure, Graphics graphics, Insets insets) {
		...
		graphics.setForegroundColor(registry
				.get(IBPELUIConstants.COLOR_COMPOSITE_ACTIVITY_BORDER));
		if (!isCollapsed()) {
			// Paint the round rectangle around the lower part of the figure
			// (surrounding the
			// children figures).
			graphics.drawRoundRectangle(expandedBounds,
					IBPELUIConstants.ARC_WIDTH, IBPELUIConstants.ARC_WIDTH);
		}
	...
	/**
	 * Compute the insets. We only know how to do this for expanded borders, so
	 * delegate to super in the collapsed state.
	 */
	@Override
	public Insets getInsets(IFigure figure) {
		...
		Insets result = new Insets(vBorderInsets + rectLabelBorder.height,
				hBorderInsets, vBorderInsets, hBorderInsets);
