...
import org.eclipse.bpel.ui.editparts.figures.CollapsableFlowContainerFigure;
...
public class FlowEditPart extends CollapsableEditPart {
	...
	private CollapsableFlowContainerFigure contentFigure;
	...
	class FlowDecorationLayout extends BPELDecorationLayout {
		...
		protected Point calculateLocation(int locationHint, IFigure container,
			...
			case PositionConstants.TOP:
				// Top Center
				return new Point(area.x + area.width / 2 - childDimension.width
						/ 2, area.y + FlowBorder.LINE_WIDTH);
	...
	public Label getLabelFigure() {
		return getFlowBorder().getLabel();
	}

	/**
	 * @author hahnml
	 */
	private FlowBorder getFlowBorder() {
		return (FlowBorder) getContentPane().getBorder();
	}
	...
	public void setCollapsed(boolean collapsed) {
		...
		if (isCollapsed()) {
			...
			IFigure[] children = (IFigure[]) figure.getChildren().toArray(
					new IFigure[0]);
	...
	protected IFigure createFigure() {
		...
		this.contentFigure = new CollapsableFlowContainerFigure(getModel(),
				getImage(), getLabel());
		...
		return editPartMarkerDecorator.createFigure(contentFigure);
	}
	...
	public void refreshVisuals() {
		...
		FlowBorder border = getFlowBorder();
		border.setName(getLabel());

		// @hahnml: Color the figure with the activity state
		this.contentFigure.setBackgroundColor(AgoraHelper
				.colorFigure(getModel()));
		...
		for (int i = 0; i < markers.length; i++) {
			...
			if (value.equals(IBPELUIConstants.MARKER_ANCHORPOINT_DRAWER_TOP)) {
				if (marker.getAttribute(
						IModelMarkerConstants.DECORATION_MARKER_VISIBLE_ATTR,
						true)) {
					int priority = marker
							.getAttribute(
	...
	public boolean detectImpendingCycle(EditPart sourceNode,
			...
			if (object instanceof BPELEditPart) {
				...
				for (Iterator it2 = ((BPELEditPart) object)
					...
					if (targetObject instanceof LinkEditPart) {
						...
						EditPartNode target = partsToNodes.get(linkEditPart
								.getTarget());
						if (target != null) {
		...
		if (source != null) {
			EditPartNode target = partsToNodes.get(potentialDest);
			if (target != null) {
				source.addEdge(new EditPartEdge(source, target));
			}
		}
