...
public class BPELReader {
	...
	//@hahnml
	protected Resource debugResource;
	...
	public void read(Resource processResource, IFile modelFile, ResourceSet resourceSet) {
		...
		//@hahnml
		IPath debugPath = modelFile.getFullPath().removeFileExtension().addFileExtension(IBPELUIConstants.EXTENSION_DEBUG);
		URI debugUri = URI.createPlatformResourceURI(debugPath.toString());
		IFile debugFile = ResourcesPlugin.getWorkspace().getRoot().getFile(debugPath);
		...
		//@hahnml
		try {
			this.debugResource = resourceSet.getResource(debugUri, debugFile.exists());
			if (this.debugResource != null) {
				this.debug = (Debug) this.debugResource.getContents().get(0); 
			}
		} catch (Exception e) {
			BPELUIPlugin.log(e);
		}
		...
		//@hahnml
		if (debug == null) {
			debug = DebugFactory.eINSTANCE.createDebug();
			debug.setProcessName(process.getName());
			debug.setProcessPath(modelFile.getLocation().toOSString());
			if (debugResource == null) {
				debugResource = resourceSet.createResource(debugUri);
			}
			debugResource.getContents().clear();
