...
import org.eclipse.bpel.ui.agora.AgoraHelper;
...
import org.eclipse.bpel.ui.editparts.figures.CollapsableContainerWithCounterFigure;
...
public abstract class CollapsableEditPart extends CompositeActivityEditPart
	...
	public Label getLabelFigure() {
		if (isGenericContainerBorder()) {
			ContainerBorder border = (ContainerBorder) (this.getContentPane()
					.getBorder());
	...
	protected IFigure createFigure() {
		...
		editPartMarkerDecorator = new BPELEditPartMarkerDecorator(
				(EObject) getModel(), new CollapsableDecorationLayout(
						image.getBounds().width));
		editPartMarkerDecorator
				.addMarkerMotionListener(getMarkerMotionListener());

		IFigure figure = getNewContentPane(editPartMarkerDecorator
				.getDecorationLayer());
		this.contentFigure = figure;

		figure.setForegroundColor(BPELUIPlugin.INSTANCE.getColorRegistry().get(
				IBPELUIConstants.COLOR_BLACK));
		if (isCollapsed()) {
	...
	protected IFigure getNewContentPane(Layer layer) {
		CollapsableContainerFigure fig = null;

		// @hahnml: Create a new figure to show the loop count for the
		// activities ForEach, While and RepeatUntil.
		if (this.getModel() instanceof ForEach
				|| this.getModel() instanceof While
				|| this.getModel() instanceof RepeatUntil) {
			fig = new CollapsableContainerWithCounterFigure(getModel(), image,
					getLabel());
			fig.addMouseMotionListener(getMouseMotionListener());
			fig.setEditPart(this);
		} else {
			fig = new CollapsableContainerFigure(getModel(), image, getLabel());
			fig.addMouseMotionListener(getMouseMotionListener());
			fig.setEditPart(this);
		}
	...
	public void setCollapsed(boolean collapsed) {
		...
		if (isCollapsed()) {
			...
			/*
			 * // Manually remove the rest of the children for (Object o :
			 * figure.getChildren()) { figure.remove((IFigure)o); }
			 */
			...
			/*
			 * // Manually remove the children for (Object o :
			 * figure.getChildren()) { figure.remove((IFigure)o); }
			 */
	...
	protected void addCollapsedContents(IFigure figure) {
		...
		if (!isGenericContainerBorder()) {
			LeafBorder lBorder = new LeafBorder(figure, true);
	...
	public void refreshVisuals() {
		...
		if (!isGenericContainerBorder()) {
			...
			// @hahnml: Handle the counter update
			if (this.contentFigure instanceof CollapsableContainerWithCounterFigure) {
				CollapsableContainerWithCounterFigure figure = (CollapsableContainerWithCounterFigure) this.contentFigure;
				figure.setName(getLabel());

				String counter = "";
				MonitorManager manager = MonitoringProvider.getInstance()
						.getMonitorManager(
								BPELUtils.getProcess(this.getActivity()));
				if (manager != null) {
					counter = manager.getMonitoringHelper().getLoopCounter(getModel());
				}

				figure.setCounter(counter);
				figure.revalidate();
				figure.setBackgroundColor(AgoraHelper.colorFigure(getModel()));
			} else {
				getContentFigure().setName(getLabel());
				getContentFigure().revalidate();

				// added by aeichel
				getContentFigure().setBackgroundColor(
						AgoraHelper.colorFigure(getModel()));
			}
		}
	...
	protected void refreshDrawerImages() {
		...
		for (IMarker marker : holder.getMarkers(getActivity())) {
			...
			if (marker.getAttribute(
					IModelMarkerConstants.DECORATION_MARKER_VISIBLE_ATTR, true) == false) {
				continue;
			}
			...
			if (value.equals(IBPELUIConstants.MARKER_ANCHORPOINT_DRAWER_TOP)) {
				int priority = marker.getAttribute(
				...
				if (priority > topMarkerPriority) {
					...
					.equals(IBPELUIConstants.MARKER_ANCHORPOINT_DRAWER_BOTTOM)) {
				int priority = marker.getAttribute(
						IModelMarkerConstants.DECORATION_MARKER_PRIORITY_ATTR,
						0);
	...
	public ConnectionAnchor getConnectionAnchor(int location) {
		...
		case CenteredConnectionAnchor.LEFT:
			return new CenteredConnectionAnchor(getFigure(),
