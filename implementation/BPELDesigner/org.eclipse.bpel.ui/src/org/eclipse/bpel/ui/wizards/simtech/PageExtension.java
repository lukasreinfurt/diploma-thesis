...
public class PageExtension extends FragmentWizardPage {

	private TableViewer viewer = null;
	
	public PageExtension(IWorkbench workbench, IStructuredSelection selection,
			WizardModel model) {
		super(workbench, selection, model, "Page5",
				"Handle Extension Conflicts", "Solve the extension conflicts");
	}
	...
	public void createControl(Composite parent) {
		...
		group.setText("Existing process extensions");
		...
		List list = new List(group, SWT.SINGLE | SWT.BORDER | SWT.V_SCROLL
				| SWT.H_SCROLL);
		if (this.model.process.getExtensions() != null) {
			list.setItems(FragmentUtils.getNameArrayFromEbjectList(
					this.model.process.getExtensions().getChildren(),
					BPELPackage.Literals.EXTENSION__NAMESPACE));
		}
		...
		ConflictTable confTable = new ConflictTable(
				ConflictTable.TableType.EXTENSION, tableViewer);
		...
		tableViewer.setInput(FragmentUtils.getUtils(null).getConflictsOfType(
				BPELPackage.Literals.EXTENSION));
