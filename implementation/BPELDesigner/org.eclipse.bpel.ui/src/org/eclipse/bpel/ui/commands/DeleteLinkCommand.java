...
public class DeleteLinkCommand extends AutoUndoCommand {
	...
	// NOTE: By selecting an activity and link at the same time, the user can
	// run
	// a GEF DeleteAction which results in two DeleteLinkCommands for the same
	// link
	...
	public void doExecute() {
		...
		for (int i = 0; i < target.length; i++) {
			if (target[i] != null)
				FlowLinkUtil.removeActivityTarget((Target) target[i]);
		}

		if (flow != null)
			FlowLinkUtil.removeFlowLink(flow, link);

		// @hahnml: Check if a breakpoint is registered for this link to delete
		// the breakpoint
		ProcessManager manager = MonitoringProvider.getInstance()
				.getActiveProcessManager();
		if (manager != null) {
			Breakpoint breakpoint = manager.getDebugManager().getBreakpoint(
					link.getXPath());
			if (breakpoint != null) {
				Breakpoints breakpoints = (Breakpoints) breakpoint.eContainer();

				manager.getDebugManager().removeBreakpoint(breakpoints,
						breakpoint);
			}
		}
	}
